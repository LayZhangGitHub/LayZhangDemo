...
@interface CaptureView() {
    CVOpenGLESTextureRef _vidoTexture;
...
- (void)render {
    ...
    GLfloat attrArr[] =
    {
        ...
        //        1.0f, 0.0f, -1.0f,   0.0f, -1.0f, -1.0f,  1.0f, 0.0f, // 右下
        //        0.0f, 1.0f, -1.0f,   -1.0f, 0.0f, -1.0f,  0.0f, 1.0f, // 左上
        //        0.0f, 0.0f, -1.0f,   -1.0f,-1.0f, -1.0f,  0.0f, 0.0f, // 左下
    ...
    //    GLuint position1 = glGetAttribLocation(self.myProgram, "position1");
    //    glVertexAttribPointer(position1, 3, GL_FLOAT, GL_FALSE, sizeof(GLfloat) * 8, (float *)NULL + 3);
...
- (GLuint)setupTexture:(NSString *)fileName {
    ...
    //    GLuint buffer;
    GLuint buffer = glGetUniformLocation(self.myProgram, "colorMap0");
...
- (void)displayPixelBuffer:(CVPixelBufferRef)pixelBuffer {
    ...
    if (pixelBuffer != NULL) {
        ...
        if ([EAGLContext currentContext] != _myContext) {
            [EAGLContext setCurrentContext:_myContext]; // 非常重要的一行代码
        }
        ...
        glBindTexture(CVOpenGLESTextureGetTarget(_vidoTexture), CVOpenGLESTextureGetName(_vidoTexture));
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
