...
- (id)initWithTitle:(NSString *)title message:(NSString *)message containerView:(UIView *)containerView delegate:(id <ZLAlertViewDelegate>)delegate confirmButtonTitle:(NSString *)confirmButtonTitle otherButtonTitles:(NSArray *)otherButtonTitles
{
    ...
    if (self) {
        ...
        if (title) {
            ...
            self.titleLabel.font = ZLBoldFont(17);
            self.titleLabel.backgroundColor = [UIColor clearColor];
            self.titleLabel.text = title;
            self.titleLabel.textAlignment = NSTextAlignmentCenter;
            self.titleLabel.numberOfLines = 0;
            self.titleLabel.textColor = ZLGray(58);
        ...
        if (message) {
            ...
            self.messageLabel.font = ZLNormalFont(15);
            
            CGSize size = [message sizeWithUIFont:ZLNormalFont(15) forWidth:self.messageLabel.width];
        ...
        if (numberOfButton > 0) {
            ...
            if (numberOfButton == 2) {
                ...
                if (!IsEmptyString(confirmButtonTitle)) {
                    ...
                    UIView *contentBtnSegementLine = [[UIView alloc] initWithFrame:CGRectMake(0, totalHeight, ALERT_WIDTH, 1/[[UIScreen mainScreen] scale])];
                    contentBtnSegementLine.backgroundColor = ZLRGB(228, 228, 228);
                    [self addSubview:contentBtnSegementLine];
